package com.whyuan.$6utils.config;

import org.apache.commons.lang.math.NumberUtils;

import java.util.HashMap;
import java.util.Map;
import java.util.Properties;

//配置对象根类
//注意key对应的value值为：Object
// 方法类型一公共方法：获取Object:string,int,long,double,boolean,int[]
// 方法类型二静态方法：Map,Properties,kv串->BaseConfiguration
public class BaseConfiguration extends HashMap<String, Object> {
    private static final String TRUE_STR = "true";
    private static final String FALSE_STR = "false";
    public static final String METRICS_ENABLED         = "metrics.enabled";
    public static final String METRICS_REPORTER        = "metrics.reporter";
    public static final String METRICS_INTERVAL_VALUE = "metrics.interval.value";
    public static final String METRICS_INTERVAL_UNIT  = "metrics.interval.unit";
    public static final String METRICS_OUTPUT          = "metrics.output";

    //获取key对应的value:String类型
    public String getString(String key) {
        String val = null;
        Object obj = get(key);
        if (null != obj) {
            if (obj instanceof String){
                val = (String)obj;
            } else {
                val = obj.toString();
            }
        } else {
            throw new IllegalArgumentException("Nothing found in configuration for " + key);
        }
        return val;
    }

    //获取key对应的value:String类型，参数非法则赋：默认值
    public String getString(String key, String def) {
        String val = null;
        try {
            val = getString(key);
        } catch (IllegalArgumentException ex) {
            val = def;
        }
        return val;
    }
    //获取key对应的value:int类型
    public int getInt(String key) {
        int val = 0;
        Object obj = get(key);
        
        if (null != obj) {
        if (obj instanceof Integer) {
            val = (Integer)obj;
        } else if (obj instanceof Number) {
            val = ((Number)obj).intValue();
        } else if (obj instanceof String) {
                try {
                    val = Integer.parseInt((String)obj);
                } catch (NumberFormatException ex) {
                    throw new IllegalArgumentException("Value for configuration key " + key + " cannot be parsed to an Integer", ex);
                }
            } else {
                throw new IllegalArgumentException("Integer value not found in configuration for " + key);
            }
        } else {
            throw new IllegalArgumentException("Nothing found in configuration for " + key);
        }
        
        return val;
    }

    //获取key对应的value:int类型，参数非法则赋：默认值
    public int getInt(String key, int def) {
        int val = 0;
        try {
            val = getInt(key);
        } catch (Exception ex) {
            val = def;
        }
        return val;
    }

    //获取key对应的value:long类型
    public long getLong(String key) {
        long val = 0;
        Object obj = get(key);
        if (null != obj) {
            if (obj instanceof Long) {
                val = (Long)obj;
            } else if (obj instanceof String) {
                try {
                    val = Long.parseLong((String)obj);
                } catch (NumberFormatException ex) {
                    throw new IllegalArgumentException("String value not found in configuration for " + key);
                }
            } else {
                throw new IllegalArgumentException("String value not found  in configuration for " + key);
            }
        } else {
            throw new IllegalArgumentException("Nothing found in configuration for " + key);
        }
        return val;
    }

    //获取key对应的value:long类型，参数非法则赋：默认值
    public long getLong(String key, long def) {
        long val = 0;
        try {
            val = getLong(key);
        } catch (Exception ex) {
            val = def;
        }
        return val;
    }

    //获取key对应的value:Double类型
    public double getDouble(String key) {
        double  val = 0;
        Object obj = get(key);
        if (null != obj) {
            if (obj instanceof Double) {
                val = (Double)obj;
            } else if (obj instanceof String) {
                try {
                    val = Double.parseDouble((String)obj);
                } catch (NumberFormatException ex) {
                    throw new IllegalArgumentException("String value not found in configuration for " + key);
                }
            } else {
                throw new IllegalArgumentException("String value not found in configuration for " + key);
            }
        } else {
            throw new IllegalArgumentException("Nothing found in configuration for " + key);
        }
        return val;
    }

    //获取key对应的value:Double类型，参数非法则赋：默认值
    public double getDouble(String key, double def) {
        double val = 0;
        try {
            val = getDouble(key);
        } catch (Exception ex) {
            val = def;
        }
        return val;
    }

    //获取key对应的value:Boolean类型
    public boolean getBoolean(String key) {
        boolean val = false;
        Object obj = get(key);
        if (null != obj) {
            if (obj instanceof Boolean) {
                val = (Boolean)obj;
            } else if (obj instanceof String) {
                val = Boolean.parseBoolean((String)obj);
            } else {
                throw new IllegalArgumentException("Boolean value not found  in configuration  for " + key);
            }
        } else {
            throw new IllegalArgumentException("Nothing found in configuration for " + key);
        }
        return val;
    }

    //获取key对应的value:Boolean类型，参数非法则赋：默认值
    public boolean getBoolean(String key, boolean def) {
        boolean val = false;
        try {
            val = getBoolean(key);
        } catch (Exception ex) {
            val = def;
        }
        return val;
    }

    //获取Key对应的value：int[]类型，int1,int2,int3,...
    public int[] getIntArray(String key, String separator) {
        String value   = getString(key);
        String[] items = value.split(separator);
        int[] values   = new int[items.length];

        for (int i=0; i<items.length; i++) {
            try {
                values[i] = Integer.parseInt(items[i]);
            } catch (NumberFormatException ex) {
                throw new IllegalArgumentException("Value for configuration key "
                        + key + " cannot be parsed to an Integer array", ex);
            }
        }
        return values;
    }

    //获取Key对应的value：int[]类型，int1,int2,int3,...，参数非法则赋：默认值
    public int[] getIntArray(String key, String separator, int[] def) {
        int[] values = null;
        try {
            values = getIntArray(key, separator);
        } catch (IllegalArgumentException ex) {
            values = def;
        }
        return values;
    }

    //默认分割符号为,
    public int[] getIntArray(String key, int[] def) {
        return getIntArray(key, ",", def);
    }

    //判断该Configuration存在该Key
    public boolean exists(String key) {
        return containsKey(key);
    }

    //将Map转为BaseConfiguration
    public static BaseConfiguration fromMap(Map map) {
        BaseConfiguration config = new BaseConfiguration();
        
        for (Object k : map.keySet()) {
            String key   = (String) k;
            Object value = map.get(key);
            
            if (value instanceof String) {
                String str = (String) value;
                
                if (DataTypeUtils.isInteger(str)) {
                    config.put(key, Integer.parseInt(str));
                } else if (NumberUtils.isNumber(str)) {
                    config.put(key, Double.parseDouble(str));
                } else if ("true".equals(value) || "false".equals(value)) {
                    config.put(key, Boolean.parseBoolean(str));
                } else {
                    config.put(key, value);
                }
            } else {
                config.put(key, value);
            }
        }
        
        return config;
    }
    //将Properties转为BaseConfiguration
    public static BaseConfiguration fromProperties(Properties properties) {
        BaseConfiguration config = new BaseConfiguration();

        for (String key : properties.stringPropertyNames()) {
            config.put(key, parseString(properties.getProperty(key)));
        }
        
        return config;
    }
    //将k1=v1,k2=v2,...的字符串转为Map,再转换为BaseConfiguration
    public static BaseConfiguration fromStr(String str) {
        Map<String, String> map = strToMap(str);
        BaseConfiguration config = new BaseConfiguration();
        
        for (String key : map.keySet()) {
            config.put(key, parseString(map.get(key)));
        }
        
        return config;
    }
    //将k1=v1,k2=v2,...的字符串转为Map
    public static Map<String, String> strToMap(String str) {
        Map<String, String> map = new HashMap<>();
        String[] arguments = str.split(",");
        
        for (String arg : arguments) {
            String[] kv = arg.split("=");
            map.put(kv[0].trim(), kv[1].trim());
        }
        
        return map;
    }

    //将字符换转为包装类型
    private static Object parseString(String value) {
        if (DataTypeUtils.isInteger(value)) {
            try{
                return Integer.parseInt(value);
            }catch (NumberFormatException e){
                return Long.parseLong(value);
            }
        } else if (NumberUtils.isNumber(value)) {
            return Double.parseDouble(value);
        } else if (TRUE_STR.equals(value) || FALSE_STR.equals(value)) {
            return Boolean.parseBoolean(value);
        }
        return value;
    }
}
